...
export function Bloom( levels,helper,baseFBO,renderer ) {
	...
	this.helper.attach( this.fbos[0], 'highlight' );

	for( var j = 0; j < this.levels; j++ ) {
		var fbo = this.createFBO();
		this.fbos.push( fbo );
		this.helper.attach( fbo, 'blur.' + j + '.h' );
		var fbo = this.createFBO();
		this.fbos.push( fbo );
		this.helper.attach( fbo, 'blur.' + j + '.v' );
	}
	this.fbos.push( this.createFBO() );
	...
	this.bloomShader = new THREE.RawShaderMaterial({
		uniforms: {
			...
			base: { type: 't', value: this.baseFBO.texture },
...
Bloom.prototype.setSize = function(w, h) {
	...
	this.helper.refreshFBO( this.fbos[0] );
	...
	for( var j = 1; j < this.levels * 2; j+= 2) {
		...
		this.helper.refreshFBO( this.fbos[j] );
		this.fbos[ j + 1 ].setSize( tw, th );
		this.helper.refreshFBO( this.fbos[j+1] );
		tw /= 2; th /= 2;
		tw = Math.round( tw );
		th = Math.round( th );
	}
...
Bloom.prototype.render = function() {
	...
	this.renderer.render(this.orthoScene,this.orthoCamera, this.fbos[ 0 ] );
	...
	for( var j = 1; j < this.levels * 2; j+= 2 ){
		...
		this.renderer.render(this.orthoScene,this.orthoCamera, this.fbos[ j ] );
		...
		this.renderer.render(this.orthoScene,this.orthoCamera, this.fbos[ j + 1 ] );
